...
export async function GET() {
  ...
  if (!supabase) {
    ...
    return NextResponse.json({ ok: false, error: 'missing_env', hasUrl, hasKey }, { status: 500 });
  }
  ...
  return NextResponse.json({ ok: true, tables: { requests: hasRequests, events: hasEvents } });
}
