...
describe('Requests moderation (merge/mute)', () => {
  ...
  const create = await createRequest(buildRequest('POST', 'http://localhost/api/requests', { track_id:'m1', title:'Mute Song', artists:'Band' }) as unknown as Request);
  ...
  it('merge incrementa duplicates senza target e poi con target', async () => {
    // primo request
  const r1 = await createRequest(buildRequest('POST', 'http://localhost/api/requests', { track_id:'x1', title:'Song X', artists:'Artist' }) as unknown as Request);
  ...
  const msj = await parseJSON(mergeSelf as Response);
    ...
    // seconda request
  const r2 = await createRequest(buildRequest('POST', 'http://localhost/api/requests', { track_id:'x1', title:'Song X', artists:'Artist' }) as unknown as Request);
